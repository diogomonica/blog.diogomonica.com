---
title: "Hitless TLS Certificate Rotation in Go"
description: "Hitless TLS certificate rotation is critical to continue our quest of reducing certificate expiration times, while keeping our sanity intact."
date: "2017-01-11"
published: "2017-01-11T08:37:11.000-08:00"
slug: "hitless-tls-certificate-rotation-in-go"
tags: ["docker", "tls", "golang", "swarm", "rotation", "MTLS"]
deadImages: []
---

One of the core security goals of Docker's [Swarm mode](https://docs.docker.com/engine/swarm/) is to be secure by default. To achieve that, when [a new Swarm gets created](https://docs.docker.com/engine/reference/commandline/swarm_init/) it generates a self-signed Certificate Authority (CA) and issues short-lived[[1]](#fn1) certificates to every node, allowing the use of [Mutually Authenticated TLS](https://en.wikipedia.org/wiki/Mutual_authentication) for node-to-node communications.

![Mutual TLS between Swarm nodes.](/content/images/2017/01/swarm-with-mutual-tls-simple.png)
...
In fact, if we do a rolling deploy of our application—by adding new application instances with the new certificate before shutting down the instances with the old certificate—we can achieve hitless rotation[[2]](#fn2) and none of the incoming requests to the application will be dropped.

![Rotation between App versions A and B, using a Load Balancer.](/content/images/2017/01/load-balancer-hitless-rotation-1.png)
...
![Swarm Architecture.](/content/images/2017/01/swarm-raft-cluster-1.png)
...
![Beginning of TLS handshake](/content/images/2017/01/begining-tls-handshake-1.png)
...
![Certificate Selection during handshake](/content/images/2017/01/certificate-selection-during-handshake-1.png)

Using `GetCertificate` is easy. The first thing we need to do is to create a `struct` that implements the `GetCertificate(clientHello *tls.ClientHelloInfo)` method[[3]](#fn3).
...
![Golang application changes the certificate currently being served.](/content/images/2017/01/golang-new-certificate-being-served.png)
...
1. The default expiration time is three months, but can be brought all the way down to one hour. [↩︎](#fnref1)
2. Assuming that we wait for all the connections from old instances to terminate. [↩︎](#fnref2)
3. I chose to use a Mutex for readability. Technically, we could have made the code shorter by using atomic.LoadPointer() instead. [↩︎](#fnref3)
